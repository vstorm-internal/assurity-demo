...
def run(**kwargs):
    ...
    if output_directory_str is None:
        ...
        logger.info("Output directory not provided. Converted files will be saved in the same location as source files.")
    else:
    ...
    try:
        ...
        logger.info("--------------------------------")
        logger.info(f"Convert and Prepare Files aborted due to error during file scanning at {script_end_dt.strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        for tif_file_path_obj in tqdm(tif_files, desc="Converting TIFF to PDF", unit="file"):
            try:
                ...
                if skip_existing_files and output_pdf_target_path.exists():
                    logger.info(f"Skipping file '{tif_file_path_obj}' because it already exists at '{output_pdf_target_path}'")
                    files_skipped += 1
                    continue
