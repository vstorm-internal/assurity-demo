...
from assurity_poc.models.output import AdjudicationOutput
...
def run(**kwargs):
    ...
    logger.info(f"Ground truth data loaded. {len(gt_df)} original rows, {len(gt_df_deduplicated)} rows after deduplication on policy_id and claim_id.")

    if not claims_batch_df.empty:
        logger.info("Step 3: Merging OCR data with ground truth data...")
        claims_batch_df = pd.merge(
            claims_batch_df,
            gt_df_deduplicated[['policy_id', 'claim_id', 'gt_decision', 'gt_status', 'gt_status_reason', 'gt_in_data_set']],
        ...
        logger.debug(f"Unique values in 'gt_decision' in merged claims_batch_df: {claims_batch_df['gt_decision'].unique()}")
        logger.debug(f"Value counts for 'gt_decision' in merged claims_batch_df:\n{claims_batch_df['gt_decision'].value_counts(dropna=False)}")
    else:
    ...
    if not claims_batch_df.empty:
        ...
        if not claims_batch_df.empty:
            ...
            logger.info(f"Merge complete. claims_batch_df_with_ocr_results now has {len(claims_batch_df_with_ocr_results)} rows.")
            ...
            if not claims_batch_df_with_ocr_results.empty and 'adjudication_decision' in claims_batch_df_with_ocr_results.columns:
                adj_decision_counts_unfiltered = claims_batch_df_with_ocr_results['adjudication_decision'].value_counts(dropna=False)
                ...
                num_adj_missing_unfiltered = int(claims_batch_df_with_ocr_results['adjudication_decision'].isnull().sum())
                logger.info(f"Unfiltered adjudication counts (from all OCR-completed claims): PAY={num_adj_pay_unfiltered}, DENY={num_adj_deny_unfiltered}, REFER={num_adj_refer_unfiltered}, MISSING/NaN={num_adj_missing_unfiltered}")
            else:
                logger.warning("Cannot calculate unfiltered adjudication counts as claims_batch_df_with_ocr_results is empty or 'adjudication_decision' column is missing.")
            ...
            if not claims_batch_df_with_ocr_results.empty and 'gt_decision' in claims_batch_df_with_ocr_results.columns:
                ...
                gt_decision_counts_for_summary = claims_batch_df_with_ocr_results['gt_decision'].value_counts(dropna=False) # Use dropna=False to count NaNs
                ...
                gt_missing_for_all_adjudicated_summary = int(claims_batch_df_with_ocr_results['gt_decision'].isnull().sum())
                logger.info(
                ...
                logger.warning("Cannot calculate ground truth counts for summary as claims_batch_df_with_ocr_results is empty or 'gt_decision' column is missing at this stage.")
            ...
            logger.info(f"Removed {original_count_before_refer_filter - len(claims_batch_df_with_ocr_results)} 'REFER' claims. {len(claims_batch_df_with_ocr_results)} claims remaining for metrics.")
            ...
            claims_batch_df_with_ocr_results = pd.DataFrame(columns=claims_batch_df.columns.tolist() + ['adjudication_decision', 'adjudication_data'] if not claims_batch_df.empty else ['policy_id', 'claim_id', 'number_of_documents_in_claim', 'did_claim_complete_ocr', 'gt_decision', 'gt_status', 'gt_status_reason', 'gt_in_data_set', 'adjudication_decision', 'adjudication_data'])
        ...
        if true_column in claims_batch_df_with_ocr_results:
            logger.info(f"Value counts for '{true_column}':\n{claims_batch_df_with_ocr_results[true_column].value_counts(dropna=False)}")
        else:
            logger.error(f"True column '{true_column}' not found in the DataFrame. Metrics calculation will be impacted.")
        
        if pred_column in claims_batch_df_with_ocr_results:
            logger.info(f"Value counts for '{pred_column}':\n{claims_batch_df_with_ocr_results[pred_column].value_counts(dropna=False)}")
        else:
            logger.error(f"Predicted column '{pred_column}' not found in the DataFrame. Metrics calculation will be impacted.")

        metrics_df = pd.DataFrame()
        if not claims_batch_df_with_ocr_results.empty and true_column in claims_batch_df_with_ocr_results and pred_column in claims_batch_df_with_ocr_results:
            ...
            logger.info(f"Removed {removed_rows_count} rows due to missing '{true_column}'. {rows_after_gt_dropna} rows remaining for metrics.")
        else:
            logger.warning("Metrics DataFrame cannot be prepared due to empty input or missing columns. Skipping metrics calculation.")
        ...
        if metrics_df.empty:
            ...
            if y_pred_original_nans > 0:
                logger.info(f"Found {y_pred_original_nans} NaN values in '{pred_column}' for rows with valid '{true_column}'. Replacing with 'NO_PREDICTION'.")
        ...
        metrics_summary_json = {
            ...
            "data_summary": {
                ...
                "claims_completed_ocr_count": len(claims_batch_df_with_ocr_results) if not claims_batch_df_with_ocr_results.empty and 'adjudication_decision' not in claims_batch_df_with_ocr_results else (len(claims_batch_df[claims_batch_df['did_claim_complete_ocr'] == True]) if 'claims_batch_df' in locals() and not claims_batch_df.empty else 0),
                "claims_after_refer_filter": len(metrics_df) + removed_rows_count if not metrics_df.empty else (len(claims_batch_df_with_ocr_results) if not claims_batch_df_with_ocr_results.empty else 0),
            },
        ...
        md_content.append(f"## Input Parameters")
        ...
        md_content.append(f"## Data Processing Summary")
        md_content.append(f"*   Initial OCR JSON files found: {metrics_summary_json['data_summary']['initial_ocr_claims_count']}")
        md_content.append(f"*   Rows in DataFrame after loading OCR data: {metrics_summary_json['data_summary']['loaded_ocr_claims_df_rows']}")
        md_content.append(f"*   Ground truth rows (before deduplication): {metrics_summary_json['data_summary']['ground_truth_rows_before_dedup']}")
        md_content.append(f"*   Ground truth rows (after deduplication): {metrics_summary_json['data_summary']['ground_truth_rows_after_dedup']}")
        md_content.append(f"*   Claims that completed OCR (before REFER filter): {metrics_summary_json['data_summary']['claims_completed_ocr_count']}")
        md_content.append(f"*   Claims after 'REFER' filter (before GT null removal): {metrics_summary_json['data_summary']['claims_after_refer_filter']}\n")

        # --- New Section: Ground Truth Data for Adjudicated Claims ---
        md_content.append(f"## Ground Truth Data (for Adjudicated Claims)")
        md_content.append(f"This section shows the ground truth distribution for all claims that completed OCR and were processed by the adjudication model (i.e., received a PAY, DENY, or REFER decision from the model).")
        ...
        total_gt_accounted_for = gt_pay_for_all_adjudicated_summary + gt_deny_for_all_adjudicated_summary + gt_missing_for_all_adjudicated_summary
        total_adjudicated_claims_for_comparison = num_adj_pay_unfiltered + num_adj_deny_unfiltered + num_adj_refer_unfiltered + num_adj_missing_unfiltered
        md_content.append(f"  (Total claims with ground truth status accounted for: {total_gt_accounted_for}. This is based on all {total_adjudicated_claims_for_comparison} adjudicated claims.)\n")

        # --- New Section: Adjudication Decision Data ---
        md_content.append(f"## Adjudication Decision Data")
        md_content.append(f"This section shows the distribution of decisions made by the adjudication model for all claims that completed OCR.")
        ...
        md_content.append(f"*   Metrics calculated on **{len(y_true_for_report)}** data points after removing {removed_rows_count} rows with missing '{true_column}'.")
        if y_pred_original_nans_for_report > 0:
            md_content.append(f"*   **{y_pred_original_nans_for_report}** NaN values in '{pred_column}' were treated as 'NO_PREDICTION'.")
        else:
            md_content.append(f"*   No NaN values in '{pred_column}' needed replacement (for rows with valid ground truth).")
        md_content.append(f"*   Unique labels considered for metrics: `{labels_for_report}`\n")

        md_content.append(f"## Overall Accuracy")
        md_content.append(f"Accuracy measures the overall correctness of the adjudication model. It is the proportion of claims (both PAY and DENY) that the model correctly classified compared to the ground truth decisions.")
        md_content.append(f"For example, an accuracy of 0.85 means that 85% of the claims processed were correctly adjudicated by the model as either PAY or DENY, matching the ground truth.")
        md_content.append(f"Accuracy: {accuracy:.4f}\n")

        md_content.append(f"## Precision per Class")
        md_content.append(f"Precision for a specific class (e.g., PAY or DENY) answers the question: \"Of all claims that the model predicted as this class, how many were actually correct?\"")
        md_content.append(f"*   **Precision (PAY):** Of all claims the model predicted as PAY, what proportion were actually PAY according to the ground truth? High precision for PAY indicates that when the model decides to PAY a claim, it is very likely correct, minimizing overpayments.")
        md_content.append(f"*   **Precision (DENY):** Of all claims the model predicted as DENY, what proportion were actually DENY according to the ground truth? High precision for DENY indicates that when the model decides to DENY a claim, it is very likely correct, minimizing incorrect denials.")
        ...
        md_content.append(f"Recall for a specific class (e.g., PAY or DENY) answers the question: \"Of all claims that truly belong to this class, how many did the model correctly identify?\"")
        md_content.append(f"*   **Recall (PAY):** Of all claims that should have been adjudicated as PAY (according to ground truth), what proportion did the model correctly identify as PAY? High recall for PAY means the model is effective at identifying most of the claims that are genuinely payable, ensuring claimants receive due benefits.")
        md_content.append(f"*   **Recall (DENY):** Of all claims that should have been adjudicated as DENY (according to ground truth), what proportion did the model correctly identify as DENY? High recall for DENY means the model is effective at identifying most of the claims that should be denied, protecting against paying out on non-covered or fraudulent claims.")
        ...
        if not metrics_df.empty and not y_true_for_report.empty: # y_true_for_report is y_true used for metrics
            ...
            def format_json_cell(data):
                ...
                try:
                    ...
                    if isinstance(data_copy, dict) and 'claim_documents' in data_copy and isinstance(data_copy['claim_documents'], list):
        ...
        md_content.append(f"*   Metrics are calculated on {len(y_true_for_report)} data points after removing rows with missing '{true_column}'.")
        if y_pred_original_nans_for_report > 0:
            md_content.append(f"*   If '{pred_column}' had NaN values for records with ground truth, they were treated as a 'NO_PREDICTION' category.")
        ...
        md_content.append(f"*   If a class has 0 support (no true instances), its recall and F1-score will be 0 (sklearn `zero_division=0` behavior).")
        md_content.append(f"*   If a class was never predicted, its precision might be 0 (sklearn `zero_division=0` behavior).")
